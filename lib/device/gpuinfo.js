    ...
    try {
        ...
        if (isWebGLSupported) {
            ...
            if (renderInfo) {
                ...
                const blacklistedGPU = some(['mali-400', 'mali-450', 'mali-470'], element => includes(info.renderer.toLowerCase(), element)), disabledGPU = some(['ati mobility radeon hd 4'], element => includes(info.renderer.toLowerCase(), element));
                if (disabledGPU) {
                    isWebGLSupported = false;
                }
...
//# sourceMappingURL=gpuinfo.js.map