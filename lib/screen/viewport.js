...
export const getViewportSize = memoize(() => {
    ...
    if (isMobile()) {
        ...
        const topWindow = get(global, 'window.top', window), width = Math.max(get(topWindow, 'document.documentElement.clientWidth', 0), topWindow.innerWidth || 0), height = Math.max(get(topWindow, 'document.documentElement.clientHeight', 0), topWindow.innerHeight || 0);
